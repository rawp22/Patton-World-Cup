...
.points { font-weight:950; } .points:not(.zero-points) { position:relative; z-index:1; } .points:not(.zero-points)::after { content:""; position:absolute; inset:-1px; border:1px solid var(--line); pointer-events:none; z-index:2; } .points.zero-points { opacity:.34; color:var(--muted); font-weight:650; } .match-card.revealed-result .points.scenario-active.zero-points { opacity:1; color:var(--gold-dark); font-weight:950; } .scenario-muted { opacity:.34; color:var(--muted); font-weight:650; } .scenario-active { color:var(--gold-dark); font-weight:950; background:#fff7dc; } .impact-head.scenario-active { background:#f1cf6a; } .impact-head.scenario-muted { background:#faf3df; }
...
def _user_knockout_brackets(leaderboard_order, users_by_id, matches, prediction_rows_by_user_match):
    ...
    def starter_flags():
        ...
        for index, match_id in enumerate(r32_left, 1):
            ...
            flags.append((match_id, match.get("team_a"), 1, 2 * index - 1, 1))
            flags.append((match_id, match.get("team_b"), 1, 2 * index, 1))
        for index, match_id in enumerate(r32_right, 1):
            match = matches_by_id.get(match_id, {})
            flags.append((match_id, match.get("team_a"), 9, 2 * index - 1, 1))
            flags.append((match_id, match.get("team_b"), 9, 2 * index, 1))
        return flags
...
def _earned_pick_class(match, prediction, team, matches_by_id):
    if not match or not match.get("result") or not prediction or not team:
        return ""
    if prediction.get("predicted_team") != team or prediction.get("prediction") != match.get("result"):
        return ""
