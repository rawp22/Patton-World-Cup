...
def fetch_fixtures(date: str, api_key: str) -> list[dict]:
    ...
    if errors:
        print(f"API returned errors for {date}: {errors}", file=sys.stderr)
...
def update_results() -> int:
    ...
    if not api_key:
        print("API_FOOTBALL_KEY is not set; skipping result fetch.")
        return 0
    ...
    for date, date_matches in sorted(dates.items()):
        ...
        print(f"Checked {date}: {len(fixtures)} fixtures for {len(date_matches)} pending matches.")
        for match in date_matches:
            ...
            updated += 1
            print(f"Updated {match['match_id']}: {match['team_a']} {goals_a}-{goals_b} {match['team_b']} ({outcome})")
